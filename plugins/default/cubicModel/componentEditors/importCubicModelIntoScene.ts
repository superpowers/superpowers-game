...
callback: (err: string, nodeId: string) => any) {
  ...
  function onAssetReceived(assetId: string, asset: CubicModelAsset) {
    ...
    projectClient.editAssetNoErrorHandling(SupClient.query.asset, "addNode", name, options, (err: string, nodeId: string) => {
      ...
      projectClient.editAssetNoErrorHandling(SupClient.query.asset, "addComponent", nodeId, "CubicModelRenderer", null, (err: string, componentId: Component) => {
        if (err != null) {
          projectClient.unsubAsset(entry.id, subscriber);
          callback(err, null);
          return;
        ...
        projectClient.editAssetNoErrorHandling(SupClient.query.asset, "editComponent", nodeId, componentId, "setProperty", "cubicModelAssetId", entry.id, (err: string) => {
          projectClient.unsubAsset(entry.id, subscriber);
