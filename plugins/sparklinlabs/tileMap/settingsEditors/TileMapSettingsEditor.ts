...
export default class TileMapSettingsEditor {
  ...
  constructor(container: HTMLDivElement, projectClient: SupClient.ProjectClient) {
    ...
    this.fields["pixelsPerUnit"].addEventListener("change", (event: any) => {
      this.projectClient.socket.emit("edit:resources", "tileMapSettings", "setProperty", "pixelsPerUnit", parseInt(event.target.value, 10), (err: string) => { if (err != null) alert(err); });
    });

    this.fields["width"].addEventListener("change", (event: any) => {
      this.projectClient.socket.emit("edit:resources", "tileMapSettings", "setProperty", "width", parseInt(event.target.value, 10), (err: string) => { if (err != null) alert(err); });
    });

    this.fields["height"].addEventListener("change", (event: any) => {
      this.projectClient.socket.emit("edit:resources", "tileMapSettings", "setProperty", "height", parseInt(event.target.value, 10), (err: string) => { if (err != null) alert(err); });
    });

    this.fields["layerDepthOffset"].addEventListener("change", (event: any) => {
      this.projectClient.socket.emit("edit:resources", "tileMapSettings", "setProperty", "layerDepthOffset", parseInt(event.target.value, 10), (err: string) => { if (err != null) alert(err); });
    });

    this.fields["grid.width"].addEventListener("change", (event: any) => {
      this.projectClient.socket.emit("edit:resources", "tileMapSettings", "setProperty", "grid.width", parseInt(event.target.value, 10), (err: string) => { if (err != null) alert(err); });
    });

    this.fields["grid.height"].addEventListener("change", (event: any) => {
      this.projectClient.socket.emit("edit:resources", "tileMapSettings", "setProperty", "grid.height", parseInt(event.target.value, 10), (err: string) => { if (err != null) alert(err); });
    });
